...
Note: More things to run ```& ".\build\bin\benchmark.exe" ``` or ```& ".\build\bin\memory_benchmark.exe" ``` or```& ".\build\bin\streams_benchmark.exe"``` or ```& ".\build\bin\matmul_benchmark.exe"``` or ```python benchmarks\gemm_benchmark.py```
