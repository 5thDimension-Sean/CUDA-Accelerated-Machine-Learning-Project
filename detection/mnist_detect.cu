...
void draw_prediction(const float* canvas, const Det* dets, int ndet,
...
void backward(const float* d_image, const float* d_target, const Net * net, const Acts * a, const Grads * g, const Back * bp, float* d_loss){
    ...
    dim3 g_fcw((1152+15)/16,(240+15)/16);
    dim3 g_fcx((1+15)/16,   (1152+15)/16);
    ...
    dim3 g_cbi3((14+15)/16,(14+15)/16,16);
    ...
    dim3 g_cbi2((31+15)/16,(31+15)/16,8);
    ...
    dim3 g_cbi1((64+15)/16,(64+15)/16,1);
...
int main(){
    ...
    float *META = (float*)malloc((size_t)N*KMAX*5 * sizeof(float));   // KMAX boxes/img
    ...
    load_bin("det_test_meta.bin", META, (size_t)N*KMAX*5);
    CUDA_CHECK(cudaMemcpy(d_X, X, (size_t)N*4096*sizeof(float), cudaMemcpyHostToDevice));

    const float CONF_THRESH = 0.5f;   // decode
    const float NMS_IOU     = 0.4f;   
    const float MATCH_IOU   = 0.5f;  

    float h_preds[240];
    Det cand[16], keep[16];
    int TP = 0, FP = 0, FN = 0, totalGT = 0;

    for (int s = 0; s < N; ++s){
        forward(d_X + (size_t)s*4096, &net, &a);
        cudaMemcpy(h_preds, a.preds, 240*sizeof(float), cudaMemcpyDeviceToHost);
        int nc = decode(h_preds, CONF_THRESH, cand);
        int nk = nms(cand, nc, NMS_IOU, keep);

        const float *m = META + (size_t)s*KMAX*5;
        bool used[16] = {false};               
        for (int k = 0; k < KMAX; ++k){
            const float *gt = m + k*5;
            if (gt[0] < 0) continue;          
            totalGT++;
            int   glabel = (int)gt[4];
            int   best   = -1; float bestiou = MATCH_IOU;
            for (int j = 0; j < nk; ++j){
                if (used[j] || keep[j].cls != glabel) continue;
                float iou = iou_xywh(keep[j].cx, keep[j].cy, keep[j].w, keep[j].h,
                                     gt[0], gt[1], gt[2], gt[3]);
        ...
        for (int j = 0; j < nk; ++j) if (!used[j]) FP++;  
    }
    float precision = TP / (float)(TP + FP + 1e-9f);
    float recall    = TP / (float)(TP + FN + 1e-9f);
    printf("TEST  TP=%d  FP=%d  FN=%d  (GT=%d)  precision=%.3f  recall=%.3f\n",
           TP, FP, FN, totalGT, precision, recall);
    ...
    for (int s = 0; s < 12; ++s){
        ...
        int nc = decode(h_preds, CONF_THRESH, cand);
        int nk = nms(cand, nc, NMS_IOU, keep);
        char path[64]; sprintf(path, "det_pred_%02d.png", s);
        draw_prediction(X + (size_t)s*4096, keep, nk, META + (size_t)s*KMAX*5, path);
    }
